...
class CalendarYvv{
	...
	startElements(){
		...
		this.diasLargo = ["Lunes","Martes","Miercoles","Jueves","Viernes","Sabado","Domingo"];
	}
	createCalendar(){
		this.startElements();
	...
	createHeaderM(){
		var cont = $("<div class='d-flex justify-content-between border align-items-center border-bottom-0 "+this.bg+" "+this.textColor+"'>");
	...
	createDaysMont(){
		...
		var emptyTag = "<div class='d-flex border flex-fill w-100 justify-content-center pt-0 pb-0 btn' style='color:transparent'>";
		...
		for(var i=0;i<this.cantDias;i++){
			...
			if(this.diaSeleccionado==fechNow){
				div = $("<div class='current-date-selected d-flex border flex-fill w-100 justify-content-center pt-0 pb-0 btn "+this.btnD+"' data-date='"+fechNow+"'>").html(i+1);
			}
